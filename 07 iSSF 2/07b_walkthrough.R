...
#     Wetland 1/2 x grassland
#   Nighttime:
#     Forest 1/3x grassland 
#     Wetland 1/2x grassland 
...
beta_wetland_day = log(1/2)
beta_forest_night = log(1/3)
beta_wetland_night = log(1/2)
...
names(hab_kern_day) <- "day"
values(hab_kern_night) <- hab_vals$w_prime_night
names(hab_kern_night) <- "night"
# Plot
plot(c(hab_kern_day, hab_kern_night))
...
# Let's assume our turn angles are also affected by time of day, with
# turn angles concentrated around 0 at night (moving) and around +/- pi
...
# to. Let's say we have a GPS fix every hour and we want two months of data. 
...
dat$x <- mean(ext(hab))[1]
dat$y <- mean(ext(hab))[2]
...
# Johannes introduced in module 08.
...
for (i in 2:nrow(dat)) {
  ...
  k <- abs(get(paste0("kappa_", tod_st)))
  # Mean of von Mises
  mu <- get(paste0("mu_", tod_st))
...
# We did fairly well. The true values for the forest betas are a 
# little more extreme than what we estimated, but we did a good job
# considering the complexity of our model, our sample size, and the
# boundary effect.
...
# In our model, the beta for sl is involved in an interaction with tod.
...
# Now how about the turn-angle distribution? It also depends on time of day.

cos_ta_day <- b[["tod_start_day:cos_ta_"]]

ta_distr_day <- update_vonmises(ta_distr(issf), 
                                beta_cos_ta = cos_ta_day)

cos_ta_night <- b[["tod_start_night:cos_ta_"]]
...
data.frame(tod = c("day", "night"),
  ...
  mutate(truth = c(kappa_day, kappa_night))

# Also very good, especially considering the boundary effect!
...
# Combine and plot
(lr_lc_day_plot <- bind_rows(log_rss_lc_day$df, log_rss_lc_night$df) %>% 
